...
class OrderService
{
    ...
    {
        $productsOrder = $this->getProdutosByOrder($order['products'] ?? []);
        ...
        $order = $this->orderRepository->createNewOrder($identify, $total, $status,
            $tenantId, $comment, $clientId, $tableId);
    ...
    private function getIdentifyOrder(int $qtdCaracters = 8)
    {
        ...
        $identify = substr(str_shuffle($characters), 0, $qtdCaracters);

        if ($this->orderRepository->getOrderByIdentify($identify)) {
            $this->getIdentifyOrder($qtdCaracters + 1);
        }

        return $identify;
    }

    private function getProdutosByOrder(array $productsOrder): array
    {
        $products = [];
